...
$oktaOrg = getenv('oktaOrg'); # copy this from the okta developer account profile
...
// Handle logout from this application
if (isset($_GET['logout'])) {
    // Clear all variables stored in the PHP session
    $_SESSION = [];
...
if(isset($_GET['code'])) {
    ...
    if(
        ...
        !hash_equals($_SESSION['state'], $_GET['state'])
    ) {
    ...
    if(isset($token->active) && $token->active == 1) {
...
if(!isset($_SESSION['username'])) {
    ...
    echo '<p><a href="' . htmlspecialchars($authorize_url) . '">Log In</a></p>';
}
